...
router.get('/:certificateId', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
router.get('/public/:certificateNumber', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/verify/:certificateNumber', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const certificate = await prisma.certificate.findUnique({
      where: { certificateNumber: req.params.certificateNumber },
      include: {
...
router.post('/:certificateId/share', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const certificate = await prisma.certificate.findFirst({
      where: {
        id: req.params.certificateId,
